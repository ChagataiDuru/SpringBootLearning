...
public class DemoApplication {
	...
	@Bean
	public CommandLineRunner commandLineRunner(IUserRepository IUserRepository, ICourseRepository iCourseRepository, AcademicRepository academicRepository){
		return runner -> {
			createThings(IUserRepository,iCourseRepository,academicRepository);
		};
	}

	public void createThings(IUserRepository IUserRepository, ICourseRepository iCourseRepository, AcademicRepository academicRepository) {

		for(int i = 0; i < 5;i++){
			System.out.println("Creating new Student Object");
			User tempUser = new Student("Student"+ i,"A"+ i,"student@ozu.edu.tr","S029021");
			System.out.println("Saving Student...");
			IUserRepository.save(tempUser);
		...
		List<User> users = IUserRepository.findStudentsWithOzuEmail();
